...
This tutorial showcases VASCA's main statistics computation based
on synthetic data. The simple simulation framework allows for the
variation of data points, flux uncertainties, number of flare events
as well as intrinsic variability parameters like amplitude and period.
...
class SimulationParams:
    ...
    flare_amplitude: Optional[float] = None
    flare_decay: Optional[float] = 0.5
    # Trend parameters
...
def simulate_stellar_light_curve(params: SimulationParams) -> tuple:
    ...
    flux_errors = params.base_flux * params.measurement_error_fraction * np.ones(params.n_points)
    ...
    if params.n_flares is not None and params.flare_amplitude is not None:
        flare_indices = np.random.choice(params.n_points, params.n_flares, replace=False)
        for idx in flare_indices:
            flare_profile = params.flare_amplitude * np.exp(-params.flare_decay * np.abs(times - times[idx]))
...
SCENARIOS = {
    "Quiet Star": SimulationParams(
        measurement_error_fraction=0.05
    ),
    
    "Pulsating Star": SimulationParams(
        period=15,
        amplitude=0.20,
        measurement_error_fraction=0.03
    ),
    ...
    "Low-Amplitude Variable": SimulationParams(
        period=25,
        amplitude=0.03,
        measurement_error_fraction=0.01
    ),
    ...
    "Secular Trend": SimulationParams(
        slope=-1.0,
        measurement_error_fraction=0.03
    )
}
...
def interpret_statistics(scenario_name: str, stats: Dict) -> str:
    ...
    interpretations = {
        "Quiet Star": 
            f"Non-variable star showing noise-dominated behavior:\n"
        ...
        "Pulsating Star":
            f"Clear periodic variability detected:\n"
        ...
        "Flare Star":
            f"Episodic flare behavior detected:\n"
        ...
        "Low-Amplitude Variable":
            f"Subtle variability detected due to precise measurements:\n"
        ...
        "Noisy Variable":
            f"Variability partially masked by measurement errors:\n"
        ...
        "Secular Trend":
            f"Systematic brightness change detected:\n"
    ...
    return interpretations.get(scenario_name, f"No interpretation available\n{pformat(stats, sort_dicts=False)}")

def visualize_results(results: List[AnalysisResult], save_path: str = None):
    """
    ...
    for i, result in enumerate(results):
        ...
        ax1.errorbar(result.times, result.fluxes, yerr=result.errors, 
                    fmt='o', color='blue', alpha=0.6, label='Observations')
        ax1.axhline(y=result.stats['wght_mean'], color='r', linestyle='--',
        ...
        ax2.axis('off')
        ax2.text(0, 0.95, result.interpretation, 
